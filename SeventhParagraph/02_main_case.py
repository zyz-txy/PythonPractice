...
#创建与AI大模型交互的客户端对象
client = OpenAI(api_key=os.environ.get("DEEPSEEK_API_KEY"), base_url="https://api.deepseek.com", timeout=10)

#数据模型
class ApiResponse(BaseModel):
    code: int
    message: str
    data: Any #任意类型数据，用于返回数据
...
def get_sessions() -> ApiResponse:
    ...
    session_ids.sort(reverse=True)  # 按时间降序排序
    #3返回数据
    return ApiResponse(code=200, message="会话列表获取成功", data=session_ids)

#加载指定会话信息
@app.get("/api/sessions/{session_id}")#路径参数，将获取的会话ID赋值给session_id
...
#全局异常处理器：把后端异常以JSON形式返回，方便前端提示具体错误
@app.exception_handler(Exception)
def handle_exception(request: Request, exc: Exception):
    print(f"处理异常, 请求路径: {request.url}, 捕获到异常: {exc}")
    return JSONResponse(content={"code": 500, "message": f"服务器内部错误: {exc}", "data": None})

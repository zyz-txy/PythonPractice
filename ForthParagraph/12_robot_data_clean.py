...
def get_movie_cost_time(movie_cost_times):
    ...
    #统一转化为分钟
    hours = re.search(r"(\d+)h", movie_cost_time)
    minutes = re.search(r"(\d+)m", movie_cost_time)
...
def get_movie_info(movie_info_url):
    ...
    movie_info =  {
        ...
        "时长（分钟）": get_movie_cost_time(movie_cost_times),
...
def save_all_movies(all_movies):
    with open(MOVIE_LIST_FILE, mode='w', newline='', encoding='utf-8') as csvfile:
        #创建csv写入对象
        csv_writer = csv.DictWriter(csvfile, fieldnames=['电影名', '年份', '上映时间', '类型', '时长（分钟）', '评分', '语言', '导演', '作者', '宣传语', '简介'])
        #写入表头
